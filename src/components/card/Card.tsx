      ...
      >
        <div className={styled.additionalInfo}>
          ...
          <div style={{ paddingBottom: "16px" }}>
            {primaryTopics ? (
              <div>
                <p className="headlines-s" style={{ paddingBottom: "8px" }}>
                  {`${t("main themes")}:`}
                </p>
                <div>
                  {primaryTopics.map((topic, index) => {
                    if (index + 1 === primaryTopics.length) {
                      return (
                        <p className="body-s" key={"primaryTopic " + index}>
                          {`${makeFirstLetterUppercase(topic)}.`}
                        </p>
                      );
                    }
                    return (
                      ...
                      >{`${makeFirstLetterUppercase(topic)}, `}</p>
                    );
                  })}
                </div>
              </div>
              ...
              <div>
                ...
                <div>
                  {secondaryTopics.map((topic, index) => {
                    if (index + 1 === primaryTopics.length) {
                      return (
                        <p className="body-s" key={"secondaryTopic " + index}>
                          {`${makeFirstLetterUppercase(topic)}.`}
                        </p>
                      );
                    }
        ...
        <div className={styled.card__content}>
          {!recomendation && (
            <div
              ...
              style={recomendation ? { right: "1rem" } : undefined}
          ...
          <div>
            ...
            {!recomendation && (
              <div className={styled.language}>
                <span className="body-l">{learningLanguage}</span>
                <div className="divider__dot" />
                <span className={`body-l ${styled.scrollable_text}`}>
                  {t(`selectedLanguageLevel.${languageLevel}`)}
                </span>
              </div>
            )}
          </div>
            ...
            style={
              recomendation
                ? {
                    ...
                    marginTop: recomendation ? "0.5rem" : undefined,
                  }
                : undefined
            }
          >
            {!recomendation && (
              <div className={styled.difficulty}>
                ...
                <Rating rating={acceptance ? acceptance : 0} isComplexity />
              </div>
            )}
